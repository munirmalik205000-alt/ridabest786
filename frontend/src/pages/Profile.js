...
import { ArrowLeft, User, IdentificationCard, LockKey, Upload, UserCircle } from '@phosphor-icons/react';
...
const Profile = () => {
  ...
  if (!profile) return <div className="min-h-screen flex items-center justify-center bg-background">Loading...</div>;

  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      <div className="bg-brand-gradient text-white p-6 shadow-lg">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold flex-1" data-testid="profile-title">My Profile</h1>
          <ThemeToggle className="bg-white/10 border-white/20" />
        </div>
      </div>

      <div className="max-w-4xl mx-auto p-6 space-y-6">
        {/* Personal Information */}
        <Card className="border-border rounded-2xl" data-testid="personal-info-card">
          <CardHeader>
        ...
        {/* Transaction PIN */}
        <Card className="border-border rounded-2xl" data-testid="pin-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent>
            {user?.has_pin ? (
              <div className="flex items-center justify-between p-4 bg-primary/10 rounded-xl">
                <p className="text-primary font-medium">✓ PIN is already set</p>
                <Button
                  variant="outline"
                  className="rounded-xl"
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-center">
                        ...
                        >
                          <InputOTPGroup>
                            <InputOTPSlot index={0} className="w-14 h-14 text-2xl border-2 border-primary/30 rounded-xl" />
                            <InputOTPSlot index={1} className="w-14 h-14 text-2xl border-2 border-primary/30 rounded-xl" />
                            <InputOTPSlot index={2} className="w-14 h-14 text-2xl border-2 border-primary/30 rounded-xl" />
                            <InputOTPSlot index={3} className="w-14 h-14 text-2xl border-2 border-primary/30 rounded-xl" />
                          </InputOTPGroup>
                        </InputOTP>
                      </div>
                      {pinStep === 2 && (
                        <p className="text-center text-xs text-muted-foreground">
                          Re-enter same 4 digits to confirm
                        </p>
                      )}
                    </div>
                    {pinStep === 2 && (
                      <div className="grid grid-cols-2 gap-2">
                        <Button
                          variant="outline"
                          onClick={() => { setPinStep(1); setPin(''); setConfirmPin(''); }}
                        ...
                        <Button
                          ...
                          className="btn-brand rounded-xl h-11"
        ...
        {/* KYC Documents */}
        <Card className="border-border rounded-2xl" data-testid="kyc-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
        ...
        {/* Nominee Details */}
        <Card className="border-border rounded-2xl" data-testid="nominee-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
