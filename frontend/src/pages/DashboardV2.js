    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" className="hover:bg-muted rounded-full w-10 h-10 p-0" data-testid="hamburger-menu-btn">
                  <List size={22} weight="bold" />
                </Button>
              </SheetTrigger>
              <SheetContent side="left" className="w-[85vw] max-w-sm bg-card text-foreground" data-testid="sidebar-menu">
                <SheetHeader className="border-b border-border pb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-brand-gradient flex items-center justify-center text-white font-black text-xl shadow-md">
                      {user?.name?.charAt(0)}
                    </div>
                  ...
                  {menuItems.map((item, idx) => (
                    <motion.div key={item.path} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.03 }}>
                      <Button
                        ...
                        variant="ghost"
                        className="w-full justify-start text-base hover:bg-primary/10 rounded-xl h-12"
                        data-testid={item.testId}
                      >
                        <item.icon size={22} className="mr-3 text-primary" weight="duotone" />
                        <span className="font-medium">{item.label}</span>
                      </Button>
                    </motion.div>
                  ))}
                  <div className="pt-3 mt-3 border-t border-border">
                    <Button onClick={logout} variant="ghost" className="w-full justify-start text-base text-rose-600 hover:bg-rose-500/10 rounded-xl h-12" data-testid="menu-logout">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            {user?.role === 'admin' && (
              <Button
                ...
                className="bg-muted border border-border hover:bg-muted/70 text-xs h-9 rounded-full px-3"
                data-testid="admin-panel-btn"
              >
                <ShieldCheck size={16} weight="fill" className="mr-1 text-primary" /> <span className="hidden sm:inline">Admin</span>
              </Button>
            )}
            <ThemeToggle />
            <NotificationBell />
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 lg:px-6 pt-4 sm:pt-5 pb-8">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
          ...
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-extrabold tracking-tight" data-testid="welcome-text">
            {user?.name} 👋
          </h1>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="relative overflow-hidden rounded-3xl bg-brand-gradient p-5 sm:p-7 shadow-2xl shadow-primary/20" data-testid="hero-balance-card">
            <div className="absolute -top-16 -right-16 w-48 h-48 sm:w-60 sm:h-60 bg-white/10 rounded-full blur-3xl" />
            <div className="absolute -bottom-16 -left-16 w-48 h-48 sm:w-60 sm:h-60 bg-white/10 rounded-full blur-3xl" />
            ...
            <div className="relative mt-2">
              <span className="text-3xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight block" data-testid="hero-total-balance">
            ...
            <div className="relative flex flex-wrap items-center gap-2 mt-5">
              <Button onClick={() => navigate('/add-fund')} className="rounded-full bg-white text-primary hover:bg-white/90 font-bold h-10 px-4 sm:px-5 text-sm shadow-md" data-testid="hero-add-fund">
              ...
              <Button onClick={() => navigate('/packages')} className="rounded-full bg-amber-400 text-slate-900 hover:bg-amber-300 font-bold h-10 px-4 sm:px-5 text-sm shadow-md" data-testid="hero-packages">
        ...
        <section className="mt-6 sm:mt-8">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base sm:text-lg font-bold tracking-tight">Quick Actions</h3>
          </div>
          <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-8 gap-2 sm:gap-3">
            {quickActions.map((qa, i) => (
              <motion.button
                ...
                className="group flex flex-col items-center gap-1.5 sm:gap-2 bg-card border border-border hover:border-primary hover:shadow-md rounded-xl sm:rounded-2xl p-2.5 sm:p-3 transition"
              ...
              >
                ...
                <span className="text-[10px] sm:text-[11px] font-semibold text-foreground text-center leading-tight">{qa.label}</span>
              </motion.button>
            ))}
          </div>
        </section>
        ...
        <section className="mt-6 sm:mt-8">
          <div className="flex items-center justify-between mb-3">
            <div className="min-w-0">
              <h3 className="text-base sm:text-lg font-bold tracking-tight">Investment Packages</h3>
              <p className="text-xs text-muted-foreground">Activate once. Earn daily. Withdraw anytime.</p>
            </div>
            <Button variant="ghost" onClick={() => navigate('/packages')} className="text-primary hover:bg-primary/10 rounded-full text-xs sm:text-sm flex-shrink-0" data-testid="see-all-packages">
        ...
        <section className="mt-6 sm:mt-8">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base sm:text-lg font-bold tracking-tight">Your Stats</h3>
          </div>
...
const StatCard = ({ title, value, icon: Icon, tone = 'emerald', delay = 0, testId }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}>
      <div className="bg-card border border-border rounded-2xl p-3 sm:p-4 hover:border-primary/40 hover:shadow-md transition" data-testid={testId}>
        ...
        <div className="text-lg sm:text-xl lg:text-2xl font-extrabold mt-0.5">{value}</div>
      </div>
    </motion.div>
  );
};
